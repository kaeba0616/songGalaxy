...
**은하 탐색**
- 30,000+곡이 12개 성단(장르 그룹)으로 뭉친 구형 3D 은하 — 성운 글로우, 배경 별 반짝임, 미니맵, 커서 방향 줌
- 3단계 탐색: 멀리서 성단 이름표 → 다가가면 세부 장르 → 진입하면 곡 제목 (좌표는 불변, 카메라만 이동)
- 성단/장르 라벨 클릭 드릴다운 + 하단 인기곡 카드 캐러셀
- **곡 클릭 = 비행 + 그 곡의 장르 카드 목록이 열리고 해당 곡 포커스 · 더블클릭 = 여기에 재생까지**
  (은하·행성 공통 제스처. 인기순 150곡 밖의 곡은 목록 맨 앞에 삽입)

**듣기 (장르 라디오)**
- 카드 ▶ 30초 미리듣기 — 곡이 끝나면 다음 곡 자동 재생, 재생 중 카드가 따라 스크롤
- 볼륨 슬라이더·음소거 토글 (설정은 저장되어 재방문에도 유지)
- 카드 목록 ▼/▲ 접기·펼치기 (접으면 헤더 바만 남음), 좌우 버튼·가로 스크롤
- 곡 상세 페이지: YouTube 영상, 가사 전문(LRCLIB, 출처 표기), 가수 정보(MusicBrainz), 앨범아트

**내 별 (은하 주민)**
- Google 로그인 후 좋아요 5곡 → 취향 중심점에 금빛 별 탄생 🌟
- 좋아요할 때마다 별이 새 취향 중심으로 부드럽게 이동, "✦ 내 별" 버튼으로 포커싱
- `/me`: 좋아요 목록·성단 분포·별 상태·행성 테마 설정

**개인 행성 — 별 클릭 착륙**
- 유저 별을 클릭하면 접근 비행 → 금빛 플래시 → **동물의 숲풍 밤하늘**로 착륙 (연출 중 클릭 스킵)
- 그라데이션 하늘 돔 + 곡선 지평선·언덕 실루엣 + 잔별 900개 + 유성 🌠 + 발밑에서 배어나는 별빛
- 좋아요 전곡이 **장르 색 별**로 하늘에 뜸 — 최근 곡은 눈높이, 오래된 곡일수록 정수리 쪽
- 별 호버 강조·선택 링, 착륙하면 자동 라디오 시작, 행성 정보 패널(좋아요 수·성단 분포)
- **행성 테마** 4종(자정·오로라·노을·달빛) — 방문자에게도 주인의 테마로 보임
- `/planet/[id]` 공유 딥링크 + "🔗 링크 복사" — 내 행성을 링크로 보낼 수 있음
