...
async function main(): Promise<void> {
  ...
  const [artistLimit, perArtist, totalLimit] = process.argv.slice(2).map(Number);
  const stats = await expandArtists({
    artistLimit: artistLimit || 50,
    perArtist: perArtist || 3,
    totalLimit: totalLimit || 150,
  ...
  console.log(`  이미 있음: ${stats.duplicate} / 좌표 실패: ${stats.noPlacement}`);
