...
| 곡 데이터 원천(아티스트 확장) | 외부 Last.fm API → `src/server/artist-expand.ts` | `songs` 테이블 (source=`lastfm`, batch=`lastfm-expand-<날짜>`) | 은하에 있는 가수의 대표곡 중 없는 곡을 채운다. 장르는 그 가수의 기존 곡에서 상속(추정하지 않음), 인기도는 청취자 수 로그 스케일. `LASTFM_API_KEY` 없으면 아무 동작도 하지 않음. CLI: `npm run expand:artists`. 롤백=배치 삭제 |
