...
| YouTube 영상 ID | 외부 YouTube Data API | `songs.youtube_video_id` (파생 캐시, `src/server/youtube.ts`) | 곡당 1회 검색(쿼터 100곡/일). 키 없으면 검색 링크 폴백. **영상 ID를 새로 찾는 곳은 목록에 곡을 담을 때(`addSongToPlaylist`) 한 곳뿐이다** — 다른 데서 부르면 하루 쿼터가 마른다 |
